...
function decrypt(){
	global $signature, $privateKey, $hash, $privateKeyFolder, $arrPackages, $encrypted;
	
	$strEncrypted = $_POST['encrypted'];
	...
	$decoded = json_decode(base64_decode($strEncrypted), true);
	
	$rsaKeys = $decoded['keys'];
	$arrArrayKeys = array_keys($rsaKeys);
	$myKeyID = $arrArrayKeys[0];
	$aesKey = $decoded['keys'][$myKeyID];
	
	//Decrypt AES Key
	$decryptedAESKey = "";
	$a = openssl_private_decrypt(base64_decode($aesKey), $decryptedAESKey, $pkeyid, OPENSSL_PKCS1_OAEP_PADDING);
	
	$decrypted = "";
	$output = openssl_decrypt(base64_decode($decoded['cipher']), 'AES-256-CBC', $decryptedAESKey, OPENSSL_RAW_DATA, base64_decode($decoded['iv']));
...
<form method="post" action="">
					...
					<textarea name="encrypted" style="width:95%; height: 200px;" required><?php echo $encrypted;?></textarea>
					
					
					<div class="buttonbar">
						<button type="submit" name="sign">Decrypt</button>
					</div>
					
					<textarea name="decrypted" style="width:95%; height: 200px;" readonly><?php echo $signature;?></textarea>
					</form>
						</div>
					</div>
				</div>
				<div id="footer">
					<div>
					<a href="https://eqdkp-plus.eu/about">EQDKP Plus Decryptor © 2006 - <?php echo date('Y', time()); ?> by EQDKP Plus Development-Team</a>
					</div>
				</div>
			</div>
			</form>
