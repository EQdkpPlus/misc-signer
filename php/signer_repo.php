...
function build_queries(){
	...
	foreach($arrPackages as $key => $val){
		...
		echo "UPDATE eqdkp23_repo_packages SET signature = '".$arrPackages[$key]['sig']."',signature_sha256 = '".$arrPackages[$key]['sig_sha256']."',status=2 WHERE id=".$arrPackages[$key]['id']."; \n";
	}
}
...
function build_json(){
	...
	foreach($arrPackages as $key => $val){
		...
		$arrOut[(int)$arrPackages[$key]['id']] = array($arrPackages[$key]['sig'], $arrPackages[$key]['sig_sha256']);
	}
...
#installer {
	width: 1200px;
		...
		<meta http-equiv="Content-Type" content="text/html; charset=utf-8" />
		<title>Repository Package Signer</title>
	</head>
	...
	<body>
		<div id="handler">
			...
			<div id="outerWrapper">
				<div id="header">
					<div id="headerInner">
						<div class="logoContainer">
							<div id="logo"></div>
							<div id="logotext">Repository Package Signer</div>
						</div>
					</div>
				</div>
				<div id="installer">
					<div id="main">
						...
						if(!function_exists('openssl_sign')){
							echo "<span style=\"font-weight:bold;color:red;font-size:20px;\">WARNING: openssl_sign() is missing on your system. Please make sure you enabled openSSL-Support for php.<br /></span>";
						}
					?>
					...
					<table width="100%">
						...
						<?php
							foreach($arrPackages as $val){
								echo "<tr><td>".$val['id']."</td><td>".(($val['category'] == 8) ? '<span style="color:red;">'.htmlentities($val['name']).'</span>' : htmlentities($val['name']))."</td><td><input type=\"text\" onclick=\"this.select()\" readonly=\"readonly\" value=\"".((isset($val['sig'])) ? $val['sig'] : '')."\" style=\"width: 97%\"  title=\"Signature SHA1-Hash\"//><div style='color:#aba9a9;'>SHA1: ".$val['hash']."</div>
		
		</td><td><input type=\"text\" onclick=\"this.select()\" readonly=\"readonly\" value=\"".((isset($val['sig_sha256'])) ? $val['sig_sha256'] : '')."\" style=\"width: 97%\" title=\"Signature SHA256-Hash\"/><div style='color:#aba9a9;'>SHA256: ".$val['hash_sha256']."</div>
				...
				<div id="footer">
					<div>
					<a href="https://eqdkp-plus.eu/about">EQDKP Plus Repository Package Signer © 2006 - <?php echo date('Y', time()); ?> by EQDKP Plus Development-Team</a>
					</div>
				</div>
			</div>
			</form>
